...
def confirm_user(update: Update, context: CallbackContext):
    ...
    if CONFIRM == "Yes":
        ...
        if check_user_database(username):
            update.message.reply_text(
                f"User {username} already exists.", reply_markup=ReplyKeyboardRemove()
            )
            os.remove("cache.txt")
        ...
        update.message.reply_text(
            f"Adding {username} to database...", reply_markup=ReplyKeyboardRemove()
        )
...
def start_stream(update: Update, context: CallbackContext):
    ...
    stream = subprocess.Popen(["python3.9", "stream.py", chat_id])
...
def stop_stream(update: Update, context: CallbackContext):
    ...
    data = subprocess.run(["pgrep", "python", "-a"], capture_output=True, text=True)
    ...
    for processes in out:
        if "stream.py" in processes:
            ...
            logging.info(f"{update.message.from_user.username} stopped the stream")
            return
